...
local function GetSocietyMoney(job)
    return exports["nfs-billing"]:getSocietyBalance(('society_%s'):format(job)) or 0
end
...
RegisterCallback("qbx:services:addMoney", function(source, amount)
    ...
    local success = exports["nfs-billing"]:depositSociety(('society_%s'):format(jobName), amount)
...
RegisterCallback("qbx:services:removeMoney", function(source, amount)
    ...
    if not job.isboss or amount < 0 or GetSocietyMoney(jobName) < amount then
        ...
        local success = exports["nfs-billing"]:withdrawSociety(('society_%s'):format(jobName), amount)
